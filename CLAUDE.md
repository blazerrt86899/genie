...
| Styling | Tailwind CSS v3 | `darkMode: "class"` + CSS-var tokens. **"Nebula" palette**: `--brand` violet + `--brand-2` cyan (every brand gradient is violet→cyan); `--glow` / `--glow-2` feed the aurora; **`--sidebar`** is a distinct surface. Sidebar nav items + chat rows are `rounded-full` pills (`bg-accent` on hover/active). Light mode is a soft tinted-white ground with white `--card` surfaces. |
...
GET    /api/v1/users/me/usage          → {token_budget, tokens_used, messages, conversations}  (Settings → Usage; sums `messages.metadata.total_tokens`, `chars/4` fallback for pre-tracking msgs)
...
| Settings modal / usage | `components/settings/SettingsModal.tsx` (⌘,) — General/Account/Usage; usage from `GET /users/me/usage` (`message_repo.usage_totals`, `chars/4` fallback) |
